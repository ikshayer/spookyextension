...
//   const fogImg = fogImages[Math.floor(Math.random() * fogImages.length)];
...
//   const audioFile = audioTracks[Math.floor(Math.random() * audioTracks.length)];
...
//   setInterval(() => {
//     timeElapsed++;
//     // Gradually increase intensity over 20 minutes
//     hauntLevel = Math.min(10, Math.floor(timeElapsed / 120));
//   }, 1000);
...
//     background-image: url(${chrome.runtime.getURL(imageFolder + fogImg)});
...
//   .shadow {
//     position: absolute; inset: 0;
//     background-image: url(${chrome.runtime.getURL(imageFolder + shadowImg)});
//     background-size: cover;
//     opacity: 0.06;
...
//     right: -80px;
//     bottom: 10%;
//     background: url(${chrome.runtime.getURL("silhouette.png")}) no-repeat center/contain;
//     opacity: 0.03;
//     filter: blur(1px);
//     transform: scaleX(-1);
//     z-index: 2147483647;
//     pointer-events:none;
//     transition: transform 2.6s ease, opacity 1.8s ease;
//   }
//   .screen-tear {
//     position: fixed;
//     inset: 0;
...
//   src.src = chrome.runtime.getURL(audioFolder + audioFile);
//   src.type = "audio/mpeg";
//   ambient.appendChild(src);
//   ambient.volume = 0.15;
...
//     fogLayer.style.opacity = 0.12 + (level * 0.15);
//     shadowLayer.style.opacity = 0.06 + (level * 0.12);
//     vignetteLayer.style.opacity = 0.9 + (level * 0.1);
//     ambient.volume = Math.min(0.35, 0.15 + (level * 0.2));
//   }

//   setInterval(updateAmbientIntensity, 2000);
...
//     tempAudio.volume = 0.4 + Math.random() * 0.3 + (hauntLevel * 0.03);
//     try { await tempAudio.play(); } catch {}
//     setTimeout(() => tempAudio.pause(), 5000);
//   }
...
//     if (eyesVisible || hauntLevel < 5) return;
...
//     if (hauntLevel < 6) return;
    
//     const now = Date.now();
//     if (now - lastIntenseEvent < 60000) return; // Cooldown
...
//   // Schedule all events with increasing frequency
//   ambient.play().catch(() => {});
  
//   scheduleEvent(triggerGlitch, 8000, 25000, 1.5);
//   scheduleEvent(playRandomCreepySound, 15000, 35000, 1.2);
//   scheduleEvent(triggerFlash, 20000, 45000, 1.3);
//   scheduleEvent(triggerScreenTear, 30000, 60000, 1);
//   scheduleEvent(triggerInversion, 40000, 80000, 0.8);
//   scheduleEvent(corruptText, 25000, 50000, 1);
//   scheduleEvent(showCreepyText, 20000, 40000, 1.2);
//   scheduleEvent(interfereCursor, 35000, 70000, 0.9);
//   scheduleEvent(triggerBreathing, 45000, 90000, 0.7);
//   scheduleEvent(createEyeFollower, 60000, 120000, 0.5);
//   scheduleEvent(showFakeNotification, 50000, 100000, 0.8);
//   scheduleEvent(interfereScroll, 40000, 80000, 0.9);
//   scheduleEvent(triggerIntenseMoment, 90000, 180000, 0.3);
// })();